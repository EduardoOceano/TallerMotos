...
namespace TallerMotos.Controllers
{
    public class VentasTallerController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            {
                cn.CommandText = "SELECT SUM(f.total), t.idTaller, f.fecha FROM facturas f INNER JOIN talleres t ON(t.idTaller = f.idTaller) GROUP BY YEAR(f.fecha) ORDER BY t.idTaller";
